/*
-- putty-0.59/windows/cthelper/cthelper.c	1969-12-31 17:00:00.000000000 -0700
++ putty-0.59-cygterm/windows/cthelper/cthelper.c	2007-02-02 18:26:59.000000000 -0700
@ -0,0 +1,780 @@
*/
#include <stdlib.h>
#include <string.h>
...
static int
connectCygTerm(unsigned int port)
{
	int sock;
	struct sockaddr_in sa;

	DBUG_ENTER("connectCygTerm");
...
static int
setupChild(pid_t *pid, const char *term, const char *attr, char *const *argv)
{
	int master;
	char ttyname[20];

	DBUG_ENTER("setupChild");
...
static void
processMsg(Buffer b, int pty)
{
	Message m;

	DBUG_ENTER("processMsg");
...
*/
	...
	/* connect to cygterm */
	{
		...
		if (0 > (s = connectCygTerm(ct_port))) {
			DBUG_PRINT("startup", ("connectCygTerm: bad"));
	...
	/* start child process */
	if (0 > (t = setupChild(&child, argv[2], argv[3], argv + 4))) {
		DBUG_PRINT("startup", ("setupChild failed: %s", strerror(-t)));
