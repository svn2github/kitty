...
// Delai avant d'envoyer le password et d'envoyer vers le tray (automatiquement √† la connexion) (en milliseconde)
int init_delay = 2000 ;
...
// Contenu d'un script a envoyer √† l'ecran
...
// Flag pour retourner √† la Config Box en fin d'execution
...
// Flag permettant de desactiver la sauvegarde automatique des informations de connexion (user/password) √† la connexion SSH
...
#ifdef RECONNECTPORT
// Flag pour inhiber le m√©canisme de reconnexion automatique
...
void SendAutoCommand( HWND hwnd, const char * cmd ) {
	if( strlen( cmd ) > 0 ) {
		...
		if( conf_get_int(conf,CONF_protocol) != PROT_TELNET) logevent(NULL, buf ) ; // On logue que si on est pas en telnet (√† cause du password envoye en clair)
		free(buf);
...
// Procedure specifique √† la editbox multiligne (SHIFT+F8)
...
Le chemin vers l'exÈcutable WinSCP est dÈfini dans la variable WInSCPPath. Elle peut pointer sur un fichier .BAT pour passer des options supplÈmentaires.
@ECHO OFF
...
// Creer une arborescence de repertoire √† partir du registre
...
void InitWinMain( void ) {
	...
	// Initialisation des parametres √† partir du fichier kitty.ini
	LoadParameters() ;

	// Ajoute les r√©pertoires InitialDirectory et ConfigDirectory au PATH
#ifdef CYGTERMPORT
