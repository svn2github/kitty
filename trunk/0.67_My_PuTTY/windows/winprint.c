                            ...
                            int offset, int *nprinters_ptr)
{
    ...
    *buffer = sresize(*buffer, offset+512, char);
     ...
     */
    EnumPrinters(param, NULL, level, (PBYTE)((*buffer)+offset), 512,
		 &needed, &nprinters);
    ...
    *buffer = sresize(*buffer, offset+needed, char);
    ...
    if (EnumPrinters(param, NULL, level, (PBYTE)((*buffer)+offset),
...
printer_job *printer_start_job(char *printer)
{
    ...
    if (!StartDocPrinter(ret->hprinter, 1, (PBYTE)&docinfo))
	goto error;
    jobstarted = 1;
