...
static const char MESSAGE[] = "                                                                                       KiTTY software is developed by Cyd for 9bis.com, copyright © 2006-2010, thanks to Leo for bcrypt and mini libraries, thanks to all contributors                                                                                       " ;
			      ...
			      WPARAM wParam, LPARAM lParam)
{
	...
	switch (LOWORD(wParam)) {
	  ...
	  case IDA_DON:
	    /* Load web browser */
	  {	char buffer[1024]="";
		sprintf( buffer, "http://www.9bis.net/kitty/?page=Donation&zone=en" ) ;
		ShellExecute(hwnd, "open", buffer, 0, 0, SW_SHOWDEFAULT);
	  }
	    return 0;
	}
...
int do_config(void)
{
	...
	GotoConfigDirectory() ;
	//SaveRegistryKey( ) ;
	_beginthread( routine_SaveRegistryKey, 0, (void*)NULL ) ;
#endif
