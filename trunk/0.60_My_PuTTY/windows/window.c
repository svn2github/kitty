...
int xpos_init=0, ypos_init=0 ;
	if( cfg.saveonexit && (cfg.xpos>=0) && (cfg.ypos>=0) ) {
...
TrayIcone.hWnd = hwnd ;
...
#ifdef PERSOPORT
    if( !PuttyFlag )
    if( cfg.saveonexit && (xpos_init>=0) && (ypos_init>=0) ) {
	...
	if( !PuttyFlag ) {
		...
		AppendMenu(m, MF_ENABLED, IDM_TOTRAY, "Send to t&ray");
		if( cfg.alwaysontop )
	...
	if( !PuttyFlag ) {
		...
		if( cfg.initdelay != init_delay ) { init_delay = cfg.initdelay ; }
		if( cfg.bcdelay != between_char_delay ) { between_char_delay = cfg.bcdelay ; }
...
void cleanup_exit(int code)
{
	...
	if( IniFileFlag == SAVEMODE_REG ) { // Mode de sauvegarde registry
		...
		if( RegTestKey( HKEY_CURRENT_USER, "Software\\SimonTatham\\PuTTY" ) )
			RepliqueToPuTTY() ; // Sauvegarde la conf dans Putty (uniquement si elle existe)
#endif
		}
...
#define TO_CHR_Y(y) ((((y)<0 ? (y)-font_height+1: (y))-offset_height) / font_height)
	...
	}
	{
		...
		break;
	      case WM_RBUTTONDOWN:
#ifdef PERSOPORT
		if( !ProtectFlag ) {button = MBT_RIGHT;	press = 1;}
#else
		button = MBT_RIGHT;
		press = 1;
#endif
		...
		break;
	      case WM_RBUTTONUP:
#ifdef PERSOPORT
		if( !ProtectFlag ) { button = MBT_RIGHT;press = 0; }
#else
		button = MBT_RIGHT;
		press = 0;
#endif
	...
	/* HACK: PuttyTray / Nutty : END */
      ...
      case WM_SYSKEYUP:
		...
		if( ShortcutsFlag ) { if ( message == WM_KEYDOWN ) {
...
{
//#ifdef PERSOPORT
//	int res = term_data_untrusted(term, data, len);
//	if( (!PuttyFlag)&&(ScriptFileContent!=NULL) ) ManageInitScript( data, len ) ;
//	return res ;
//#else
